...
def render_html(input_path: Path, output_path: Path) -> None:
    ...
    if not cli_path.is_file():
        raise FileNotFoundError(
            f"Renderer CLI not found: {cli_path}. Build it with `npm --prefix packages/renderer run build`."
        )
...
def run_render_html(*, context: StageContext, store: JobStore) -> str:
    markdown = store.read_artifact(job_id=context.job_id, relative_path="04-wechat.md")

    with TemporaryDirectory() as temp_dir:
        input_path = Path(temp_dir) / "04-wechat.md"
        output_path = Path(temp_dir) / "05-wechat.html"
