...
title: "如何将我的 Claude Code 变成 24/7 全职开发团队（完整指南 + 代码库）"
coverImage: "01-source.assets/imgs/img-001-HHFImuGbkAAKLL4.jpg"
---

# 如何将我的 Claude Code 变成 24/7 全职开发团队（完整指南 + 代码库）

大多数人把 Claude Code 当作聊天机器人来使用。而我却把它改造成了一个全天候工作的开发团队，这个团队能记住所有内容，并且每次会话都会变得更聪明。
...
现在，Claude Code 能记住我做过的每一个决定。在我睡觉的时候，它可以并行运行五个 Agent；它会自动执行我的编码规范，无需我额外提醒。而且，每次会话结束后，它都会变得更加智能，而不是重置回初始状态。
...
这一部分非常神奇。claude-subconscious 是一个后台 Agent，它会监控你的会话、读取你的文件，并在你不知不觉中逐步建立记忆。
...
默认情况下，Claude Code 是一个通用型助手，它几乎什么都能做，但没有哪方面特别擅长。

Skills 正是解决这一问题的关键。它们是基于 Markdown 的文件，用于指导 Claude 如何按照你的要求高效地完成特定任务。

第一个每个人都应该安装的是 Superpowers。

Superpowers 拥有超过 17 万个 GitHub 星标，已正式入驻 Anthropic 插件市场。它能够将 Claude Code 从“按要求写代码”的通用助手，转变为一套完整的开发方法论。
...
## /第 4 部分 - Subagents：一个 Claude 可以变成五个

这才是真正的核心。

单个 Claude Code 会话在同一时间只能做一件事。你要它先写功能，再审查代码，然后修复 Bug，最后写文档——它会依次完成每一项任务，但到了第四项时，上下文就已经被污染了。

Subagents 则可以将工作分解开来。与其让一个超负荷的 Claude 来处理所有事情，不如组建一个由多个专业 Agent 组成的团队，每个 Agent 都有自己的上下文和单一职责。
...
## /第 6 部分 - 编排：Agent 在你睡觉时工作

这是最后一环。它让每月 20 美元的订阅变成了一种仿佛拥有一支开发团队的感觉。
...
> https://github.com/ruvnet/claude-flow——11.4 万+ 颗星，提供企业级的多 Agent 编排能力，并支持持久化内存。
...
Claude Code 订阅费用：$20/月（Pro 计划）
```

这份清单上的所有内容都是开源的。你唯一需要支付的费用就是 Claude Code 的订阅本身。
...
第二天早上：三个 PR 正在等待审查。每个 PR 都在自己的分支上，没有冲突，测试也都通过了。
...
第 2 层：Obsidian + claude-mem  — 免费（Obsidian 是免费的，相关仓库都是开源的）
第 3 层：Superpowers + Skills   — 免费（全部开源，MIT 许可证）
第 4 层：Subagents              — 免费（Markdown 文件）
第 5 层：Hooks + Commands       — 免费（全部开源）
第 6 层：claude-squad           — 免费（开源）
...
这份清单上的所有内容都是开源的。你唯一需要付费的就是 Claude 的订阅本身。

而投资回报率更是毋庸置疑。我在搭建这套系统前后分别跟踪了两周的生产力：
